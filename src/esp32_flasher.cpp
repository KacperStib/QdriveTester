...
int ESP32Flasher::flashBeginCmd(uint32_t offset,
  ...
  // ESP32-S2 expects 20-byte payload (total packet: 8 header + 20 = 28 bytes)
  uint8_t cmd_data[28] = {0};

  // Fill header (8 bytes)
  cmd_data[0] = WRITE_DIRECTION;    // direction
  cmd_data[1] = FLASH_BEGIN;        // command
  cmd_data[2] = 0x14;               // payload size = 20 bytes (0x14)
  cmd_data[3] = 0x00;               // size high byte
  cmd_data[4] = 0x00;               // checksum placeholder
  cmd_data[5] = 0x00;
  cmd_data[6] = 0x00;
  ...
  // Fill payload (20 bytes)
  memcpy(&cmd_data[8],  &erase_size,      4);  // erase size
  memcpy(&cmd_data[12], &blocks_to_write, 4);  // packet count
  memcpy(&cmd_data[16], &block_size,      4);  // block size
  memcpy(&cmd_data[20], &offset,          4);  // flash offset
  uint32_t flags = 0;
  memcpy(&cmd_data[24], &flags,           4);  // flags (only in S2+)
...
bool ESP32Flasher::espConnect(void) {
  ...
  do {
    ...
    if (get_sync_status == ERR_TIMEOUT_flash) {
...
void ESP32Flasher::espFlasherInit(void) {
  ...
  // Wejdz w tryb operatywny slavea
  digitalWrite(BOOT_PIN, HIGH);
  digitalWrite(EN_PIN, HIGH);
